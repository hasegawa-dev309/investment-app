...
if stock_code:
    ...
    try:
        ...
        st.subheader("収益性指標")
        st.write(f"PER（株価収益率）: {per:.2f}" if per else "PER（株価収益率）: データなし")
        st.write(f"ROE（自己資本利益率）: {roe * 100:.2f}%" if roe else "ROE（自己資本利益率）: データなし")

        # 財務情報の取得と処理
        st.subheader("売上高・純利益の推移")
        financials = ticker.financials.T[::-1].dropna(how="all", axis=1)
        ...
        if revenue_key in financials.columns and profit_key in financials.columns:
            revenue = financials[revenue_key].dropna() / 1e8
            net_income = financials[profit_key].dropna() / 1e8
            ...
            if len(revenue) >= 2 and revenue.iloc[0] != 0 and len(net_income) >= 2 and net_income.iloc[0] != 0:
                ...
                # 最終判断
                st.markdown("### 総合判断")
                if revenue_cagr > 0.05 and profit_cagr > 0 and (roe and roe > 0.1) and negative_years == 0:
